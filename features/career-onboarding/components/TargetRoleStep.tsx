...
import {
  buildProfileAwareJobRoleOptions,
  jobRoleIdToLabel,
  jobRoleLabelToId,
  labelsToRoleIds,
  MAX_TARGET_ROLES,
...
import { getProfileRelevantJobRoles } from '../utils/suggestJobRolesFromProfile';
...
export function TargetRoleStep({ profile, onChange }: TargetRoleStepProps) {
  ...
  const relevantRoles = useMemo(() => getProfileRelevantJobRoles(profile), [profile]);
  const options = useMemo(
    () => buildProfileAwareJobRoleOptions(relevantRoles, targetRoles),
    [relevantRoles, targetRoles]
  );
